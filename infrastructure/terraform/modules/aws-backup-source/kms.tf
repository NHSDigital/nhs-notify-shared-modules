...
data "aws_iam_policy_document" "backup_key_policy" {
  ...
  statement {
    ...
    principals {
      type        = "AWS"
      identifiers = ["arn:aws:iam::${data.aws_caller_identity.current.account_id}:root", "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/service-role/GitHub_Deploy"]
    }
    actions   = ["kms:*"]
    resources = ["*"]
  }
